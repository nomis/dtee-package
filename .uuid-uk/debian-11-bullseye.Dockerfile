...
RUN \
	--mount=type=cache,sharing=locked,target=/var/cache/apt,id=debian-bullseye-var-cache-apt \
	--mount=type=cache,sharing=locked,target=/var/lib/apt,id=debian-bullseye-var-lib-apt \
