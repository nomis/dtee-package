...
RUN \
	--mount=type=cache,sharing=locked,target=/var/cache/apt,id=debian-buster-var-cache-apt \
	--mount=type=cache,sharing=locked,target=/var/lib/apt,id=debian-buster-var-lib-apt \
	apt-get update
RUN \
	--mount=type=cache,sharing=locked,target=/var/cache/apt,id=debian-buster-var-cache-apt \
	--mount=type=cache,sharing=locked,target=/var/lib/apt,id=debian-buster-var-lib-apt \
...
RUN \
	--mount=type=cache,sharing=locked,target=/var/cache/apt,id=debian-buster-var-cache-apt \
	--mount=type=cache,sharing=locked,target=/var/lib/apt,id=debian-buster-var-lib-apt \
	apt-get install -y wget debhelper dh-virtualenv ninja-build python3-sphinx
